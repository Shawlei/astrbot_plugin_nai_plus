...
| 配置面板六大真实折叠组、预设三要素（质量前缀/正向词/负面词）联动、确认开关及聊天指令查看/删除 | v1.3.1 |
...
- **配置面板六大折叠板块（v1.3.1 追加修订）**：使用 AstrBot 原生 `type: object` + `items` 结构，基础设置、画质安全、预设管理、翻译、图生图和系统工具均可独立折叠
- **预设管理与三要素生图联动（全新 1.3.0）**：
  - 预设支持扩展质量前缀（`artist`）、正向词（`prompt`）、专属负面词（`negative`）
  - 生图时自动智能拼接预设正向词，自动优先使用预设专属负面词
  - 内置预设与自定义预设分层持久化存储，出厂内置预设绝对保护
  - 管理面板提供预设类型选择器、三要素输入和 `preset_add_confirm` 确认开关；只有打开确认并保存配置才会添加，聊天指令负责查看与删除
...
在 AstrBot 管理面板 → 插件配置中，六个顶层板块都使用原生 `type: object` + `items` 配置组，可像图生图参数一样展开或折叠。v1.3.1 追加修订仍不恢复会被当成输入框的 `section_*` 伪标题；旧版顶层平铺配置在代码侧继续兼容读取：
...
| `preset_add_confirm` | 确认添加开关；填写上方四项后开启为 `true`，再保存配置；成功后自动清空输入并复位为 `false` | `false` |
| `preset_manage_name` | 紧接添加区域，可手填/复制已保存的用户预设名称；不会直接执行操作 | 空 |
| `preset_preview` | 预览说明文本，不是实时预览；提示使用 `/nai presets <预设名>` 查看、`/nai del <预设名>` 删除 | 说明文本 |
...
> **面板添加预设流程**：填写 `preset_add_target`、名称及三要素 → 打开 `preset_add_confirm` → 点击 AstrBot 的“保存配置”。仅确认开关为 `true` 且名称非空时才保存；成功后名称与三要素自动清空，开关自动复位为 `false`，重复重载不会再次添加。`preset_preview` 只是紧随 `preset_manage_name` 的预览说明，不承诺实时展示预设内容。
