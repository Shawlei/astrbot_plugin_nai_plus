...
| **拼接预览** | 输一段正向词，直接看到**最终发给 NovelAI 的是什么**：画师串怎么拼、`-2::green::` 这种负权重挪去了哪、有没有自动补构图。用的是文本框里当前的内容，改一改看一看，满意再保存 |
| **预设管理** | 自定义预设新增 / 编辑 / 删除。内置 5 个预设只读，但可以「复制为新预设」再改，或者一键「设为默认画师串」 |
...
| **质量前缀 / 画师串** | 会拼在提示词最前面。写画师串、质量词、构图词都行 |
| **备注** | 只用于在列表里显示，可留空 |

改完**立即生效**，不用重启插件。每个条目右上角有删除按钮。
