...
- **内置 1466 条词库**：角色名 / 外观 / 场景直接查表，不走模型 —— 角色名 100% 准确，纯标签输入生图更快
...
| 现象 | 原因与解决 |
|------|-----------|
| **画风不生效、张张都是上半身** | v1.2.2 已修。此前画师串被当成独立请求参数发送、没拼进提示词，导致预设里的画风与构图标签（`perspective` 等）全部失效。升级到 v1.2.2+ 即可 |
...
| 文件 | 内容 | 条数 |
|------|------|------|
| `core/prompt_dict/characters.json` | 角色名：原神 / 崩铁 / 绝区零 / 蔚蓝档案 / 明日方舟 / FGO / 东方 Project / 经典动画 / VTuber / 战双 / 鸣潮 / 单机游戏 | **797** |
| `core/prompt_dict/appearance.json` | 发色发型 / 眼睛 / 表情 / 体型 / 服装 / 姿势 / 镜头 | **400** |
| `core/prompt_dict/scene.json` | 环境背景 / 画风 / 动作 / 构图 | **280** |
| 合计 | （去重后加载 1466 条） | **1477** |

合计 **1477 条**（去重后加载 **1466 条**）。角色名全部对照 Danbooru 官方标签，不做音译。
...
│   ├── prompt_dict/        # 内置词库（角色/外观/场景，共 1466 条）
│   │   ├── characters.json
│   │   ├── appearance.json
│   │   └── scene.json
