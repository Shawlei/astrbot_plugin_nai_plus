...
- **内置 1440 条词库**：角色名 / 外观 / 场景直接查表，不走模型 —— 角色名 100% 准确，纯标签输入生图更快
...
| `core/prompt_dict/scene.json` | 环境背景 / 画风 / 动作 / 构图 | **250** |
| 合计 | （去重后加载 1440 条） | **1447** |

合计 **1447 条**（去重后加载 **1440 条**）。角色名全部对照 Danbooru 官方标签，不做音译。
...
│   ├── prompt_dict/        # 内置词库（角色/外观/场景，共 1440 条）
│   │   ├── characters.json
│   │   ├── appearance.json
│   │   └── scene.json
