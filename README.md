...
- **内置 1578 条词库**：角色名 / 外观 / 场景直接查表，不走模型 —— 角色名 100% 准确，纯标签输入生图更快
...
| `default_artist` | 默认画师串/质量前缀 | **日系动漫风**（v1.6.0 起） |
...
| `default_preset` | 默认预设（填预设名后**每次生图自动套用它的三段**；建议从 WebUI 点「设为默认风格」设置） | 空（不使用） |
| `show_image_info` | 图片信息标签（在图片下方显示预设名和耗时） | `true` |
| `send_retry_on_timeout` | 发图超时自动重试（见下方「收到重复图片」；**默认关闭**） | `false` |
...
| `img2img_inherit_artist` | 图生图时带上画师串（见下方「两套引擎」；默认关闭，关闭时信息标签会提示「画师串未生效」） | `false` |
...
### ⚠️ 先读：文生图和图生图是**两套引擎**，别混淆

这是最容易踩坑的地方：插件里「文生图」和「图生图」走的是**完全不同的两条路**，能用的东西也不一样。

| | 文生图 | 图生图 |
| --- | --- | --- |
| 谁来画 | NovelAI（通过 Nai2API 网关） | **你自己配的渠道**（`gpt-image-2` 等） |
| 怎么触发 | 直接发指令 | **回复一张图片**再发指令 |
| 画师串 | ✅ 有效，作为独立 `artist` 参数发送（在 NovelAI 端拼在提示词最前面） | ⚠️ **默认不发送**（见下） |
| NovelAI 标签语法（`artist:xxx`、`1.2::tag::`、`{{}}`） | ✅ 有效 | ❌ 渠道多半不认 |
| 负向词 | ✅ 有效 | 取决于渠道模板，不保证 |

**为什么图生图默认不带画师串**：图生图渠道绝大多数是 OpenAI 风格接口，它们不认识 NovelAI 的画师串语法。硬拼进去，轻则无效，重则干扰模型对正常提示词的理解 —— 反而画得更差。

但插件**绝不会默默丢掉**：一旦发生「你配了画师串、图生图却没带上」，图片下方的信息标签会变成：

```
自定义画师串 | 图生图 | 相似度0.60 | 耗时8秒（画师串未生效）
```

看到「**画师串未生效**」这几个字，就说明这次画师串没参与。
想让它参与，两种办法：

1. 打开配置项 **`img2img_inherit_artist`（图生图时带上画师串）** —— 画师串会拼在提示词最前面，和文生图一致。**前提是你的渠道能识别这些标签**，否则可能适得其反
2. 用 **`--no-i2i`** 强制走文生图，让画师串在原生的 NovelAI 上生效
...
| 现象 | 原因与解决 |
|------|-----------|
| **换了画师串 / 预设，画风几乎不变** | **v1.6.2 已修**。根因：Nai2API **服务端**会把 `input.artist`（取不到就退回**服务端自己存的**默认画师串）拼在 `tag` 前面再发给 NovelAI。而旧版插件**不发 `artist`**、把画师串拼进了 `tag` —— 于是服务端每次都用自己的默认画师串（2.5D 写实）打头，把用户的设置整个压在下面。升级到 **v1.6.2+** 后，画师串改回「作为独立 `artist` 参数发送」，画风会真正随预设变化。**临时绕法**（未升级时）：去 Nai2API 面板把**服务端自己的**画师串清空，用户端的设置才会完全生效 |
...
| **图生图出来画风不对 / 画师串像没生效** | v1.4.6 起会明说。图生图走的是你自己配的渠道（`gpt-image-2` 等），**默认不继承画师串** —— 因为这类渠道不认 NovelAI 的画师串语法。如果你配了画师串，此时图片下方的信息标签会出现「（画师串未生效）」。想让它生效：打开配置项 `img2img_inherit_artist`（前提是渠道认这些标签），或者用 `--no-i2i` 强制走文生图。详见上方「两套引擎」 |
...
| `原神神里绫华穿着泳衣在沙滩玩耍` | 夹着的 `穿着` / `在` 是**连接词**，剥掉后全命中，不调模型 |
| `超级赛亚人发型`（词库没有） | **宁可整体交模型，也不做半截替换** |

> **连接词是什么**：`穿着`、`戴着`、`在`、`的`、`一个`… 这批词对画面没有任何影响，但用户写整句时天然会带上它们。词库里不可能收录它们，以前它们会让整句查不到词库、白白调一次模型。现在只要**残留全是这类词**就照常命中；残留里还有任何一个有含义的汉字（`超级赛`、`发型`），仍然整句交给模型 —— 半截替换比不替换危险得多。

### 单角色自动补人数标签

人数标签（`1girl` / `1boy` / `solo`）是 NovelAI 最重要的主体标签：没有它，模型得自己猜画里有几个人，主体经常跑偏。

翻译提示词里已经把它定成硬约束，但模型不听话是常态，所以代码层还有一道兜底：**画面里只有一个角色时自动在最前面补 `solo`**。

判定「一个角色」靠两件事：命中词库里的角色名（658 条，已剔除 `genshin impact` 这类作品名），或者标签长得像 `xxx_(作品名)` 的消歧写法。

| 输入 | 输出 |
|------|------|
| `原神神里绫华穿着泳衣在沙滩玩耍` | `solo, genshin impact, kamisato ayaka, swimsuit, beach, playing` |
| `神里绫华 雷电将军`（两个角色） | `kamisato ayaka, raiden shogun`（不补，人数不确定） |
| `1girl, 神里绫华`（已有人数标签） | `1girl, kamisato ayaka`（不重复补） |

> **为什么补的是 `solo` 不是 `1girl`**：词库里只有角色名，**没有性别信息** —— 代码不知道 `kamisato ayaka` 是女的。猜错性别比不写更糟（`1girl` 配一个男角色会让模型在两者间摇摆）。`solo` 只表达「就一个人」，不分性别，任何情况下都不会翻车。
>
> **为什么两个角色不补**：可能是 `2girls`，也可能是 `1girl, 1boy`，人数和性别都不确定，宁缺勿错。
...
| `core/prompt_dict/scene.json` | 环境背景 / 画风 / 动作 / 构图 | **279** |
| 合计 | 3 个文件 | **1578** |

合计 **1578 条**（去重后加载 **1571 条** —— 有 7 个中文键在不同分组里重复出现，值相同，属于冗余不算错）。角色名全部对照 Danbooru 官方标签，不做音译。
...
| 区块 | 干什么用 |
|------|---------|
| **画师串 / 质量前缀** | 编辑每次生图默认发送的画师串（作为独立 `artist` 参数，在 NovelAI 端拼在提示词最前面）。有字符数 / 标签数统计、未保存提示、一键追加 NovelAI 官方质量词、一键重置为默认 |
...
| **预设管理** | 自定义预设新增 / 编辑 / 删除。每个预设有**三段**：画师串 + 附带正向词 + 附带负向词（任填一段）。内置 5 个预设只读，但可以「复制为新预设」再改，或者一键「**设为默认风格**」（整个预设设为默认，三段全部生效） |
...
| 段 | 拼到哪 | 举例 |
|----|--------|------|
| **画师串** | 作为独立 `artist` 参数发送，在提示词**最前面**，替代全局画师串。可留空 = 沿用全局 | `artist:xxx, best quality, year 2025` |
...
> 正向词为什么放用户提示词**后面**：NovelAI 越靠前的标签权重越高。画师串（风格）最前，用户写的画面内容其次，预设的「常驻补充」放最后——这样你写的东西不会被预设里的固定词压住。（提示：插件侧画师串是作为**独立的 `artist` 参数**发出的，由服务端把它拼在提示词前；这里描述的是**最终提示词里的权重顺序**。）

**和其他入口的关系**：面板、聊天指令（`/nai save` 等）、配置页的「预设管理」列表，三处操作的是**同一份数据**，随便从哪边改，另外两边看到的都一样。

### 默认预设（省掉每次都写 -p）

如果某个预设是你**最常用**的，不必每次打 `-p`，把它设成「默认预设」即可：之后**每次生图都自动套用它的三段**（画师串 + 附带正向词 + 附带负向词）。

**怎么设**：面板「预设管理」里，找到那张卡，点 **「设为默认风格」**。设好之后：

- 这张卡会显示 **`默认`** 徽章，按钮变成「取消默认」（再点一下即可取消）
- 「画师串 / 质量前缀」区域顶部会显示 `当前默认预设：xxx`，一眼能看到生效的是哪个

也可以直接填配置项 `default_preset`（预设名，留空 = 不使用）—— 但**手填名字容易写错**，还是推荐从面板点按钮设置。

**临时想换别的 / 完全不用**：

| 需求 | 怎么做 | 说明 |
|------|--------|------|
| 临时换一个预设 | `/nai -p 其它预设名 ...` | 显式 `-p` **永远优先**于默认预设 |
| 本次完全不用预设 | `/nai --no-preset ...` | 只对这一次生效，默认预设本身不变 |
| 彻底不再用 | 面板点「取消默认」，或把 `default_preset` 清空 | |

举个例子：默认预设设成「夜景」之后，普通一句 `/nai 一个银发女孩` 就会自动带上夜景的正向/负向词；偶尔想画白天的场景，写 `/nai --no-preset 一个银发女孩 白天` 就绕过了它。

> 如果默认预设指向的名字**被删掉了**，插件会记一条告警并按「没有预设」继续生图 —— 删个预设不会让生图整个坏掉。
...
3. 插件详情页刷新一下再点页面
4. 看 AstrBot 日志里有没有 `[Nai WebUI]` 开头的行，注册成功会打「管理面板 API 已注册（N 个接口）」
...
如果某个预设是天天要用的，可以把它设成「**默认预设**」省掉 `-p`（见上方 WebUI 章节的「默认预设」）；临时想换别的仍用 `-p`，临时完全不用加 `--no-preset`。
...
> 不写 `-p` 就用配置里的默认画师串。**v1.6.0 起默认是「日系动漫风」**（`year 2024, anime style, illustration, ...`，刻意不含构图标签，把构图交给「自动补构图标签」兜底）。想要写实 / 2.5D 效果，用 `-p 2.5D唯美风` 切回旧风格（那串完整保留着）。

**默认预设** — 把最常用的那个预设设成默认，之后**每次生图自动套用它的三段**，不用每次写 `-p`（怎么设见上方 WebUI 章节的「[默认预设](#默认预设省掉每次都写--p)」）：

```
（已把「夜景」设为默认预设）
/nai 一个银发女孩              → 自动套用夜景的画师串 + 正向词 + 负向词
/nai -p 动漫风 一个银发女孩    → 临时换成动漫风（-p 优先于默认预设）
/nai --no-preset 一个银发女孩  → 本次完全不用预设
```
...
| 预设名 | 说明 |
|--------|------|
| 2.5D唯美风 | Nai2API **原**默认，写实 / 半写实风格（v1.6.0 起已不再是插件默认，但预设保留着，想要写实随时可切回） |
| 韩漫小清新风 | 韩式漫画清新风格 |
| 本子动漫风 | 日系本子风格，多画师混合 |
...
**参数优先级**：`artist` > `preset` > 默认预设 / 默认画师串（v1.6.0 起默认是「日系动漫风」）
...
**v1.4.5 起默认只发一次**，超时就把本地文件路径发给你（因为超时不代表图没发出去，重发会造成重复，见下一条）。处理建议：

1. 稍等几秒再发一次指令（或直接用提示里给的本地路径取图）
2. 反复出现 → 看 NapCat / QQ 客户端是否卡顿、内存是否吃紧，重启一下通常就好
3. 只在 2K / 4K 出现 → 先用普通尺寸确认发图链路正常
4. 宁可偶尔漏图也不想看到重复图 → 保持 `send_retry_on_timeout` 关闭（默认）

</details>

<details>
<summary><b>收到两张（或多张）一模一样的图</b></summary>

打开了「发图超时自动重试」（`send_retry_on_timeout`），且第 1 次发送实际上**成功了**、只是 QQ 回执慢。

关键点：**NapCat 报 `sendMsg Timeout` 不等于图没发出去**。图片常常已经送达，只是客户端返回超时；这时插件按配置又发了一次，群里就出现两张一样的图。

插件没法从超时本身判断图到底送没送出去，所以把选择权交给你：

| 设置 | 行为 | 代价 |
| --- | --- | --- |
| **关闭**（默认，推荐） | 只发一次，超时就提示并给本地路径 | 偶尔漏图（可用提示里的路径补救） |
| 开启 | 超时后隔 2 秒再发一次 | 可能收到两张重复的图 |

看到重复图 → 去插件配置页把「发图超时自动重试」关掉即可。

</details>

<details>
<summary><b>「参考图下载/解码失败：object str can't be used in 'await' expression，本次改为文生图」</b></summary>

这是 **v1.4.4 及更早版本**的 bug：图生图的参考图落地代码对一个同步函数写了 `await`，必然抛这个 `TypeError`，然后被兜底逻辑吞掉、静默降级成文生图。

也就是说在 **v1.4.5 之前，图生图一次都没成功过**，一直在悄悄按文生图出图。升级到 **v1.4.5** 即可修复（同时对下载做了线程池包装，不会再卡住事件循环）。升级后如果仍看到这条提示，说明是参考图本身取不到（比如回复的那条消息被撤回了），按提示的「可以用 `--no-i2i` 强制走文生图」处理即可。
...
<details>
<summary><b>「画出来的图里有我没写的标签」/ 想知道到底给 NovelAI 发了什么</b></summary>

日志里有两条能直接回答这个问题（**v1.6.1 起**）：

1. `[Nai2API] 提示词已直译: <你的输入> → <直译结果>` —— 直译的**完整**结果（旧版这里只打前 60 字符、还看不出被截过）
2. `[Nai2API] 预设「xxx」附带正向词已拼入: ...` / `附带负向词已拼入: ...` —— 若末尾那些标签来自某个预设的**附带词**，这里会点名是**哪个预设**、拼进了什么

配合 `[Nai2API] 提示词:` 那条最终的完整提示词，就能判断某个标签究竟来自：① 你写的（直译得到）、② 某个预设的附带词、③ **直译模型自己脑补的** —— 若它在最终提示词里出现，却**任何一条来源日志里都找不到**，那就是模型脑补（属于该改翻译提示词的情况）。

> 日志过长时按 1000 字符截断，并会**显式标出**「…（共 N 字符）」，不会再有「被砍了却看不出来」的情况。
...
│   ├── prompt_dict/        # 内置词库（角色/外观/场景，共 1578 条）
│   │   ├── characters.json
│   │   ├── appearance.json
│   │   └── scene.json
