...
- **预设管理**：自定义预设可在 **WebUI 里直接增删改**，不用改文件也不发指令，改完立即生效；与 `/nai save` 等指令读写同一份数据
...
| `custom_presets` | 预设管理（**WebUI 里直接增删改预设**，不用改文件也不发指令） | 空 |
...
| **WebUI 加的预设没出现** | 检查预设名是否含空格、是否和内置预设重名 —— 这两种情况会被跳过，日志里有 `[PresetManager] 忽略预设` 提示 |
| **用指令加的预设过一阵自己没了** | v1.3.1 已修。旧版本只写 `presets.json` 没回写配置，重载时被配置里的旧值覆盖。现在指令改完会自动回写，升级即可 |
| 重载后 WebUI 里还是旧预设 | 配置页需要手动刷新一下；预设本身已生效，可用 `/nai presets` 看当前实际生效的列表 |
| `找不到对应模板，请删除后重新添加` | WebUI 编辑器的报错，说明该条目缺 `__template_key`。删掉这条重新「添加条目」即可。正常通过编辑器加的不会出现 |
...
生图有**两条完全独立的触发路径**，黑名单必须两条都堵。这点容易漏，所以说清楚：

| 触发方式 | 路由 | 被哪层拦 |
|---|---|---|
| 唤醒词（`/nai`、`#nai`、`.`、`!`…） | `filter.command` 匹配 → 命令 handler | **第 1 层**（handler 入口静默丢弃） |
| 改了命令名（`/niu`、`/绘`…） | 同上，别名走同一主流程 | **第 1 层** |
| **@ 机器人** / @全体成员 / 引用机器人消息 | 不匹配任何命令 → 直接进 LLM | **第 2 + 3 层** |
| 群里说「画一个 xxx」（没 @） | 视唤醒配置而定，若唤醒则进 LLM | **第 2 + 3 层** |

<details>
<summary>为什么换唤醒词 / 改命令名不会绕过？</summary>

**不会。** 拦截点比"唤醒词解析"更靠前 —— AstrBot 的 `filter.command` 在**进入 handler
之前**就已经把唤醒前缀（`/`、`#`、`.`、`!`……）和命令名一起剥掉了，handler 收到的
`args` 是「命令之后的那段文本」。

所以黑名单判断只要放在 handler 入口，就**天然覆盖所有唤醒词和所有命令名** ——
插件根本不需要去识别它们是哪个，因为到不了这一步。

`command_names` 配的别名（`/niu`、`/绘`）也全部走 `_handle_generate_command`
这一个主流程，而黑名单判断是它的第一行，所以别名一个都跑不掉。

实测：`/nai`、`/niu`、`/绘`、`/draw` 四个命令名 × 各种入参形态，黑名单群全部静默拦截。

</details>

<details>
<summary>@ 机器人也能拦住吗？</summary>

**能，但靠的不是第 1 层。**

@ 和「命令」在 AstrBot 里是两条独立的路：

- **命令**：`filter.command` 匹配到命令名 → 走命令 handler → 第 1 层能拦
- **@**：在 `waking_check` 阶段被标记为 `is_at_or_wake_command = True`，但
  `wake_prefix` 是空串 —— 意味着**不匹配任何命令**，直接落到 ProcessStage 里
  「被 @ 或者通过唤醒前缀」那个分支 → `agent_sub_stage` → **直接进 LLM**

也就是说 @ 这条路**压根不经过命令 handler**，第 1 层够不着它。

拦住它的是后两层：

1. **第 3 层** `on_llm_request` —— 直接掐断 LLM 请求，模型连消息都收不到
2. **第 2 层** 工具入口 —— 万一钩子没生效，`nai_generate` 被调到时也会被拦下

实测：@ 黑名单群 → 第 3 层掐断且不发消息；第 2 层兜住；非黑名单群 @ 不受影响；
私聊 @ 不受影响；黑名单为空时谁都不拦。
...
> 需要 AstrBot **>= 4.26.0**。老版本装了也不会报错，只是没有这个页面，指令一切照旧。

**入口**：AstrBot Dashboard → 左侧「插件」→ 点本插件卡片进详情页 → 打开页面「**NovelAI 配置管理**」。

面板分四块，改完点保存**立即生效，不用重载插件**：

| 区块 | 干什么用 |
|------|---------|
| **画师串 / 质量前缀** | 编辑每次生图默认拼在最前面的那一大串。有字符数 / 标签数统计、未保存提示、一键追加 NovelAI 官方质量词、一键重置为默认 |
| **负向词** | 编辑默认负向词（Undesired Content），同样有快捷追加和重置 |
| **拼接预览** | 输一段正向词，直接看到**最终发给 NovelAI 的是什么**：画师串怎么拼、`-2::green::` 这种负权重挪去了哪、有没有自动补构图。用的是文本框里当前的内容，改一改看一看，满意再保存。还能选一个预设「模拟」，看 `/nai -p 预设名` 之后实际发出去的内容 |
| **预设管理** | 自定义预设新增 / 编辑 / 删除。每个预设有**三段**：画师串 + 附带正向词 + 附带负向词（任填一段）。内置 5 个预设只读，但可以「复制为新预设」再改，或者一键「设为默认画师串」 |

主题跟随 AstrBot 的亮 / 暗色，手机上打开也能用。

### 预设的三段是什么意思？

| 段 | 拼到哪 | 举例 |
|----|--------|------|
| **画师串** | 整条提示词**最前面**，替代全局画师串。可留空 = 沿用全局 | `artist:xxx, best quality, year 2025` |
| **附带正向词** | 用户提示词**后面**，每次用这个预设都自动带上 | `night, city lights, full body` |
| **附带负向词** | **追加**到全局负向词后面（不替换，`bad hands` 等兜底仍在） | `daylight, -2::green::` |

比如做一个「夜景」预设：画师串留空、正向词填 `night, city lights, neon`、负向词填 `daylight, sun`。之后 `/nai -p 夜景 一个银发女孩` 就会用全局画师串 + 用户提示词 + 夜景补充词，同时压掉白天元素。

> 正向词为什么放用户提示词**后面**：NovelAI 越靠前的标签权重越高。画师串（风格）最前，用户写的画面内容其次，预设的「常驻补充」放最后——这样你写的东西不会被预设里的固定词压住。

**和其他入口的关系**：面板、聊天指令（`/nai save` 等）、配置页的「预设管理」列表，三处操作的是**同一份数据**，随便从哪边改，另外两边看到的都一样。

<details>
<summary><b>页面打不开 / 提示「读取配置失败」怎么办？</b></summary>

按顺序检查：

1. AstrBot 版本是不是 >= 4.26.0（Dashboard 左下角能看到）
2. 插件是否已启用；刚更新完插件要**重载一次**，页面列表才会刷新
3. 插件详情页刷新一下再点页面
4. 看 AstrBot 日志里有没有 `[Nai WebUI]` 开头的行，注册成功会打「管理面板 API 已注册（5 个接口）」
...
除了上面的面板，你也可以在插件配置页的「预设管理」列表里改，或者用聊天指令。三种方式操作的是同一份数据。

### 怎么用

WebUI 插件配置 → **预设管理** → 点 **添加条目**：

| 字段 | 说明 |
|------|------|
| **预设名** | 调用时用的名字。**不能含空格**，不要和内置预设重名 |
| **质量前缀 / 画师串** | 拼在提示词最前面，替代全局画师串。可留空 |
| **附带正向词** | 追加到用户提示词后面的英文标签，可留空 |
| **附带负向词** | 追加到全局负向词后面，可留空 |
| **备注** | 只用于在列表里显示，可留空 |

画师串 / 正向词 / 负向词**至少填一项**。改完**立即生效**，不用重启插件。每个条目右上角有删除按钮。
...
> **为什么预设名不能含空格？** 因为 `/nai -p <预设名>` 是靠空格切分参数的，名字里有空格就切不出来了。填了带空格的名字，该条目会被跳过并在日志里提示。

> **为什么不能和内置预设重名？** 内置的 5 个（`2.5D唯美风` / `韩漫小清新风` / `本子动漫风` / `GalGame风` / `动漫风`）是 Nai2API 官方调好的。让你覆盖掉之后，`/nai -p 动漫风` 得到的就不是文档里那个效果了，出问题很难排查。所以直接拒绝并跳过，日志里会说明。

### 和指令的关系

WebUI 编辑器和指令操作的是**同一份数据**，不会打架：

| 操作方式 | 入口 |
|----------|------|
| **WebUI 编辑器** | 配置页「预设管理」增删改 |
| **聊天指令** | `/nai save` / `/nai del` / `/nai update` / `/nai presets` |
| **AI 对话** | LLM 工具 `nai_save_preset` / `nai_update_preset` / `nai_delete_preset` |

规则：

- **以 WebUI 为准** —— 同名时 WebUI 里的值胜出
- **指令改完自动回写配置** —— `/nai save` 加完预设会同步写回插件配置。不这么做的话，下次重载时配置里的旧值会把指令的改动覆盖掉，表现就是「我用指令加的预设过一阵自己没了」，很难排查
- **旧数据自动迁移** —— `presets.json` 里有、WebUI 里没有的预设（历史数据），启动时自动补进 WebUI 配置并打日志。只迁一次

<details>
<summary><b>为什么启动时选择「迁移」而不是「删除」？</b></summary>

启动时如果 `presets.json` 里有、WebUI 里没有的项，有两种可能：
...
- **启动路径**选「迁进来」—— 宁可多留一次（用户能再删），也不要静默丢数据（不可逆）
- **重载路径**提供 `exact=True` 模式走「以配置为准，不在配置里的就删」

这个区分写在 `PresetManager.reload_from_webui()` 的 docstring 里。
</details>

<details>
<summary><b>填错了会怎样？</b></summary>

配置写坏的代价必须是「**这个预设不生效**」，不能是「**插件加载失败**」—— 后者会让你连生图都用不了。所以：

| 情况 | 行为 |
|------|------|
| 预设名为空 / 含空格 / 画师串为空 / 与内置重名 | 跳过该项并记日志，其余照常 |
| 整个配置项类型不对（不是列表） | 当作空处理 |
| `presets.json` 不存在 / 是坏 JSON / 结构异常 | 当作空处理，插件正常加载 |
...
> 指令只能设置画师串；想给预设加「附带正向词 / 负向词」请到 WebUI 管理面板或配置页。
...
### nai_update_preset - 修改预设

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | string | 是 | 要修改的预设名称（内置预设无法修改） |
| `artist` | string | 是 | 新的质量前缀/画师串 |

> 三个写操作（`nai_save_preset` / `nai_update_preset` / `nai_delete_preset`）改完都会**自动回写**到插件配置的「预设管理」，所以 AI 帮用户加的预设，用户到 WebUI 里也能看到、能接着改。
...
├── pages/
│   └── nai-config/         # WebUI 管理面板（AstrBot 插件 Pages，需 AstrBot >= 4.26.0）
│       ├── index.html
│       ├── app.js
│       └── style.css
├── .astrbot-plugin/
│   └── i18n/               # 面板标题 / 描述的多语言（zh-CN / en-US）
├── tests/
│   ├── test_prompt_dict_quality.py  # 词库健康度回归测试（不依赖 AstrBot，可直接跑）
│   └── test_webui_api.py            # WebUI 面板后端回归测试（同样离线可跑）
└── persona/
