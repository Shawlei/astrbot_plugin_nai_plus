...
- **内置 1497 条词库**：角色名 / 外观 / 场景直接查表，不走模型 —— 角色名 100% 准确，纯标签输入生图更快
...
| `core/prompt_dict/appearance.json` | 发色发型 / 眼睛 / 表情 / 体型 / 服装 / 姿势 / 镜头 | **431** |
| `core/prompt_dict/scene.json` | 环境背景 / 画风 / 动作 / 构图 | **280** |
| 合计 | 3 个文件 | **1508** |

合计 **1508 条**（去重后加载 **1497 条**）。角色名全部对照 Danbooru 官方标签，不做音译。
...
│   ├── prompt_dict/        # 内置词库（角色/外观/场景，共 1497 条）
│   │   ├── characters.json
│   │   ├── appearance.json
│   │   └── scene.json
